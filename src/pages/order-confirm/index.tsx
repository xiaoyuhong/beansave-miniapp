...
export default function OrderConfirm() {
  ...
  // discount — 优惠金额
  const discount=selectedCoupon && subtotal >= selectedCoupon.minAmount ?selectedCoupon.amount : 0
  // total — 实付金额
  ...
  const  handlePay=async () => {
    ...
    if(todayQty + quantity >3){
      Taro.showToast({title:`今日已购${todayQty}杯,每天限购3杯`,icon:'none'})
      return
    }
    setPaying(true)
    ...
    try{
      await new Promise<void>((resolve,reject) => {
        Taro.showModal({
          title:'模拟支付',
          content:`确认支付￥${(total /100).toFixed(2)}？（当前为模拟支付，不会扣款））`,
          confirmText:'确认支付',
          success: (res) => {
    ...
    <View className={styles.container}>
      ...
      <View className={styles.productCard}>
        ...
        <View className={styles.productInfo}>
          ...
          <View className={styles.priceRow}>
            <Text className={styles.salePrice}>¥{(product.salePrice / 100).toFixed(0)}</Text>
            <Text className={styles.originalPrice}>原价 ¥{(product.originalPrice / 100).toFixed(0)}</Text>
          </View>
        </View>
      </View>
      ...
      <View className={styles.section}>
        ...
        <View className={styles.quantityControl}>
          <View className={[styles.quantityBtn,quantity <=1 ? styles.quantityBtnDisabled:''].join('')}
          ...
          <Text className={styles.quantityLimit}>每人限购3杯</Text>
        </View>
      </View>
        ...
        <View className={styles.section}>
          ...
          <View className={styles.tempOptionList}>
            {product.tempOptions.map( (temp)=>(
              <View 
              key={temp}
              className={[styles.tempOption,selectedTemp===temp?styles.tempOptionActive:''].join(' ')}
        ...
        <View className={styles.section}>
          ...
          {availAbleCoupons.map((coupon)=>{
            ...
            return(
              <View 
              key={coupon.id} 
              className={[styles.couponItem,isSelected? styles.couponSelected:'',!applicable? styles.couponDisabled:''].join(' ')} 
              onClick={()=> applicable && setSelectedCoupon(isSelected ? null:coupon)}
              >
                <View className={styles.couponLeft}>
                  <Text className={styles.couponAmount}>¥{(coupon.amount/100).toFixed(0)}</Text>
                  <Text className={styles.couponCondition}>满¥{(coupon.minAmount/100).toFixed(0)}可用</Text>
                </View>
                <View className={styles.couponRight}>
                  <Text className={styles.couponType}>{coupon.type=='new_user'?'新人券':'分享券'}</Text>
                  {!applicable &&(
      ...
      <View className={styles.section}>
        ...
        <View className={styles.feeList}>
          ...
          <Text className={styles.feeValue}>¥{(subtotal /100).toFixed(2)}</Text>
          </View>
          {discount>0 &&(
          <View className={styles.feeItem}>
            <Text className={styles.feeLabel}>优惠券</Text>
            <Text className={[styles.feeValue,styles.feeDiscount].join(' ')}>-¥{(discount /100).toFixed(2)}</Text>
          </View>
          )}
          <View className={[styles.feeItem,styles.feeTotal].join(' ')}>
            <Text className={styles.feeTotalLabel}>实付金额</Text>
            <Text className={styles.feeTotalValue}>¥{(total /100).toFixed(2)}</Text>
          </View>
        </View>
      </View>
      ...
      <View className={styles.tipCard}>
        <Text className={styles.tipText}>📍 到店自提：上海市松江区文汇路 718 号</Text>
        <Text className={styles.tipText}>⏰ 请于今日 20:00 前凭核销码到店取咖啡</Text>
        <Text className={styles.tipText}>💰 当前为模拟支付，不会产生真实扣款</Text>
      </View>
      ...
      <View className={styles.payBar}>
        <View className={styles.payBarLeft}>
          ...
          <Text className={styles.payBarAmount}>¥{(total /100).toFixed(2)}</Text>
        </View>
        <View 
        className={[styles.payBtn,paying? styles.payBtnLoading:''].join(' ')}
        onClick={handlePay}
        >
          <Text className={styles.payBtnText}>{paying ?'处理中…':'确认支付'}</Text>
        </View>
      </View>
    </View>
  )
